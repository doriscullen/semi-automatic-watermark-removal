...
def estimate_watermark(foldername):
	...
	images = []
	for r, dirs, files in os.walk(foldername):               #os.walk 遍历目录 见笔记
		# Get all the images
		for file in files:
			img = cv2.imread(os.sep.join([r, file]))           #os.sep.join 见笔记
			if img is not None:
				images.append(img)
			else:
...
# 	gradt = list(map(lambda x: cv2.Sobel(x, cv2.CV_64F, 1, 0, ksize=KERNEL_SIZE), images))
# 	Wm_x = np.median(np.array(gradt), axis=0)
# 	gradt = list(map(lambda x: cv2.Sobel(x, cv2.CV_64F, 0, 1, ksize=KERNEL_SIZE), images))
# 	Wm_y = np.median(np.array(gradt), axis=0)
...
def crop_watermark(gradx, grady, threshold=0.4, boundary_size=2):
	...
	W_gray = image_threshold(np.average(W_mod, axis=2), threshold=threshold)   # [r,g,b]求平均
...
def watermark_detector(img, gx, gy, thresh_low=200, thresh_high=220, printval=False):
	...
	#print("Wm in watermark_detector")
	#print(Wm)
	#Wm_print = np.expand_dims(Wm, axis=2)
...
# 		print(index)
	...
	#print("index in watermark_detector")
	#print(index)
	#print("(rect[0], rect[1])")
	#print("(",rect[0],", ",rect[1],")")
...
# 	cv2.rectangle(im, (y, x), (y+rect[1], x+rect[0]), (255, 0, 0))
	cv2.rectangle(im, (y, x), (y+rect[1], x+rect[0]), (255, 0, 0), 2)
	return (im, (x, y), (rect[0], rect[1]))   # im, start, length
